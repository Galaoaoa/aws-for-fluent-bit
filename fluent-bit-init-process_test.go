...
func TestGetECSTaskMetadata(t *testing.T) {
	...
	expectedOutput1 := ECSTaskMetadata{
		...
		ECS_CLUSTER:         "ygloaTest",
		ECS_TASK_ARN:        "arn:aws:ecs:us-west-2:546265451413:task/ygloaTest/4ca5a280e68947cd84a8357f0d008fb5",
...
func TestDownloadS3ConfigFile(t *testing.T) {
	...
	defer os.RemoveAll(s3FileFolderPath)
	...
	actualContent := readFileHelper(s3FileFolderPath + "/" + "aaa.conf")
...
func (msd *MockS3Downloader) Download(w io.WriterAt, input *s3.GetObjectInput, options ...func(*s3manager.Downloader)) (int64, error) {
	filePath := s3FileFolderPath + "/" + "aaa.conf"
