...
const s3FileFolderPathTest = "fluent-bit-init-s3-files/"
...
func TestGetECSTaskMetadata(t *testing.T) {
	...
	expectedOutput1 := ECSTaskMetadata{
		...
		ECS_TASK_ARN:        "arn:aws:ecs:us-west-2:546265451413:task/ecs-cluster/4ca5a280e68947cd84a8357f0d008fb5",
...
func TestCreateCommand(t *testing.T) {
	...
	createCommand(&fluentBitCommand, filePath)
	expectedContent += " -c " + filePath

	assert.Equal(t, fluentBitCommand, expectedContent)
}
...
func TestDownloadS3ConfigFile(t *testing.T) {
	...
	createS3ConfigFileFolder(s3FileFolderPathTest)
	defer os.RemoveAll(s3FileFolderPathTest)

	s3FilePath := "ygloa/files/aaa.conf"
	bucketName := "ygloaBucket"
	s3Downloader := MockS3Downloader{}

	downloadS3ConfigFile(&s3Downloader, s3FilePath, bucketName, s3FileFolderPathTest)

	actualContent := readFileHelper(s3FileFolderPathTest + "aaa.conf")
...
func (msd *MockS3Downloader) Download(w io.WriterAt, input *s3.GetObjectInput, options ...func(*s3manager.Downloader)) (int64, error) {
	filePath := s3FileFolderPathTest + "aaa.conf"
